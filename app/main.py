...
def get_clean_data():
    ...
    #print(data.columns)  # validar el nombre de las columnas  
    data = data.drop(['Unnamed: 32','id'], axis=1)  # Eliminar las columnas innecesarias
...
def add_sidebar():
    ...
    st.sidebar.header("Cell Nucleai Measure")
...
def get_radar_chart(input_data):
    ...
    categories = ['Raidus', 'Texture','Perimeter','Area',
                  'Smoothness', 'Compactness', 'Concavity', 
                  'Concave Points',
                  'Simmetry', 'Fractal Dimension']
    ...
    #fig.show() # en lugar de este, utilizar el de streamlit

def add_predictions(input_data):
    model = pickle.load(open('model/model.pkl', "rb"))
    scaler = pickle.load(open('model/scaler.pkl', "rb"))

    # convertir input_data en un array usando numpy
    ...
    #st.write(prediction)
    st.write("This app can assist medical professionals in making a diagnosis, but should not be used as a sustitute for a professional diagnosis")
...
def main():
    ...
    #st.write(input_data) # para visualizar el diccionario y validar que funciona

    with st.container():
        st.title("Breast Cancer Predictor")
        st.write("Please, connect this app to your cytoloty lab to help diagnose breast...")
    
    # crear dos columnas chart y prediction box

    col1,col2= st.columns([4,1]) # 4 veces mas grande que la segunda columna
